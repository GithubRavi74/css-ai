...
try:
    ...
    st.title("静态/视频 👷‍♂️ Construction Site Safety Detection - AI Site Safety Auditor")
...
if uploaded_file is not None:
    ...
    if not is_video:
        ...
        # Use two responsive side-by-side columns to group images and control scrolling
        img_col1, img_col2 = st.columns(2)
        
        with img_col1:
            st.subheader("📸 Original View")
            # Restricting width prevents the giant blown-out scaling
            st.image(image, use_container_width=False, width=550)
        ...
        if analyze_clicked:
            ...
            with img_col2:
                ...
                st.image(annotated_rgb, use_container_width=False, width=550)
            
            # Show dashboard report below the side-by-side window layout
        ...
        # Create standard layout columns for structural side-by-side placement
        ...
        if run_analysis:
            ...
            while cap.isOpened():
                ...
                video_frame_placeholder.image(last_processed_rgb, channels="RGB", width=550)
        ...
        if st.session_state.video_detections is not None:
            ...
            # Display frozen result in a balanced column setup
            res_col1, res_col2 = st.columns(2)
            if st.session_state.last_frame is not None:
                with res_col1:
                    st.subheader("🎯 Final Stream Frame")
                    st.image(st.session_state.last_frame, use_container_width=False, width=550)
